...
- **Strip a leading "The "/"A "/"An " article from the English title before slugifying.**
  Example: `The Lean Startup` → `lean-startup` (NOT `the-lean-startup`). This
  applies to both `repoName` and `folderName`. CLI also strips a leading `the-`
  defensively when parsing the response (see `AiTaskService.sanitizeRepoName`),
  but you should still emit the correct slug — if the safety net fires you'll
  see a `⚠ Stripping leading 'the-'` warning.
