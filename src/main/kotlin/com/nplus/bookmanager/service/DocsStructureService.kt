...
class DocsStructureService {
     ...
     * @param structure The DocsStructure generated by Gemini AI
    ...
    /**
     * Validate that a directory is a valid Hugo Book repository.
     */
    fun isValidHugoBookRepo(repoDir: File): Boolean {
        if (!repoDir.exists() || !repoDir.isDirectory) return false

        // Check for key Hugo Book structure files
        val hugoToml = File(repoDir, "site/hugo.toml")
        val contentDir = File(repoDir, "site/content")

        return hugoToml.exists() && contentDir.exists()
    }

    /**
     * Get the docs directory for a Hugo Book repository.
     */
    fun getDocsDir(repoDir: File): File = File(repoDir, "site/content/docs")
}
